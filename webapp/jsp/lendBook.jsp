...
<div class="container" id="container" style="width: 100%">
    ...
    <div class="row clearfix" id="body" style="display: flex; min-height: 800px">
        ...
        <div class="col-md-9 column" id="content">
            ...
            <div class="form">
                <div class="formbox">
                    <form role="form" action="${pageContext.request.contextPath}/lendBook" method="get"
...
<%--                        <input type="checkbox" id="usable" name="usable" style="width: 50px"  readonly checked/>--%>
                        ...
                        <br>
                        <button type="submit">
                            <a style="text-decoration: none; color: whitesmoke" href="${pageContext.request.contextPath}/personalRecord?userId=${sessionScope.mUser.userId}"
                               id="updateStatus"
...
<%--                        <button>--%>
<%--                            <a style="border: 0; color: whitesmoke; text-decoration:none" href="${pageContext.request.contextPath}/bookList" onClick="javascript:history.go(-1)">不借了</a>--%>
<%--                        </button>--%>
                    </form>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        $("#updateStatus").on("click", "", {}, function (event) {
            ...
            const url = "${pageContext.request.contextPath}/doBookLend"
            const data = {
                bookId: bookId,
                userId: userId,
                // status: status,
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    ...
                    if (status === true) {
                        alert("编号为" + response.bookId + "的图书借书成功！")
                    } else {
                        alert("编号为" + response.bookId + "的图书借书成功！")
                    }
                },
                error: function (data, textStatus) {
                    if (textStatus === "timeout") {
                        ...
                        alert("借书失败！错误信息:" + data.responseText)
                    }
                },
