...
<div class="container" id="container" style="width: 100%">
    <div class="row clearfix" id="head">
        ...
        background-color: #004ea0;height: 80px; display: flex; flex-direction: column; justify-content: center">
            <form class="form-inline" role="form" action="${pageContext.request.contextPath}/recordList"
    ...
    <div class="row clearfix" id="body" style="display: flex; min-height: 800px">
        ...
        <jsp:include page="left.jsp">
            <jsp:param name="navItem" value="recordList"/>
        </jsp:include>
            ...
            <div class="row clearfix">
                <div class="col-md-12 column" id="main" style="padding: 0">
                    <table class="table table-bordered"
                        ...
                        <tbody>
                        <c:forEach var="mRecords" items="${requestScope.mRecords}">
                            <tr>
                                <td style="line-height: 30px">${mRecords.id}</td>
                                <td style="line-height: 30px">${mRecords.userId}</td>
                                <td style="line-height: 30px">${mRecords.username}</td>
                                <td style="line-height: 30px">${mRecords.bookId}</td>
                                <td style="line-height: 30px">${mRecords.bookName}</td>
                                <td style="line-height: 30px">${mRecords.author}</td>
                                <td style="line-height: 30px">${mRecords.price}</td>
<%--                                <td style="line-height: 30px">${mRecords.rent}</td>--%>
                                <td style="line-height: 30px">${mRecords.lendDate}</td>
                                <td style="line-height: 30px">${mRecords.returnDate}</td>

                                <c:choose>
                                    <c:when test="${mRecords.returnDate.equals('null')}">
                                        <td style="line-height: 30px"><a
                                                href="${pageContext.request.contextPath}/recordList"
                                                id="book-lendBook-field-id"
                                                class="book-lendBook-field"
                                                data-library-book-id="${mRecords.bookId}"
                                                data-library-record-id="${mRecords.id}"
                                        >还书</a></td>
                                    </c:when>
                                    <c:otherwise>
                                ...
                                <td>
                                    <a href="${pageContext.request.contextPath}/javascript:void(0)"
                                       ...
                                       data-library-delete-user-id="${mRecords.id}">
                                        删除
                                    </a>
                                </td>
                            </tr>
